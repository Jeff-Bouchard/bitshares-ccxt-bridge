...
const route = (fn: (req: express.Request, res: express.Response) => Promise<any>) =>
  (req: express.Request, res: express.Response) => fn(req, res).catch(err => {
    console.error(err);
...
app.get('/myTrades', route(async (req, res) => {
  const { symbol, since, limit } = req.query;
  res.json(await ex.fetchMyTrades(
    symbol ? String(symbol) : undefined,
    since ? Number(since) : undefined,
    limit ? Number(limit) : undefined
  ));
...
app.post('/login', route(async (req, res) => {
  ...
  if (!account || !keyOrPassword) return res.status(400).json({ error: 'account and keyOrPassword required' });
...
app.get('/openOrders', route(async (req, res) => {
  const { symbol, since, limit } = req.query;
  res.json(await ex.fetchOpenOrders(
    symbol ? String(symbol) : undefined,
    since ? Number(since) : undefined,
    limit ? Number(limit) : undefined
  ));
...
app.post('/order', route(async (req, res) => {
  ...
  const result = await ex.createOrder(String(symbol), String(type) as any, String(side) as any, Number(amount), Number(price), params);
  res.json(result);
}));
